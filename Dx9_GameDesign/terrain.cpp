				 ...
				 float heightScale)
{
	...
	// scale heights
	_lowest = _highest = _heightmap[0];
	for (int i = 0; i < _heightmap.size(); i++)
	{
		_heightmap[i] *= heightScale;
		if (_lowest > _heightmap[i])
			_lowest = _heightmap[i];
		if(_highest<_heightmap[i])
			_highest = _heightmap[i];
	}
...
bool Terrain::computeVertices()
{
	...
	_uvScale = 50;
	float uCoordIncrementSize = 1.0f / (float)_uvScale;
	float vCoordIncrementSize = 1.0f / (float)_uvScale;
	...
	for(int z = startZ; z >= endZ; z -= _cellSpacing)
	{
		...
		for(int x = startX; x <= endX; x += _cellSpacing)
		{
			...
			v[index] = TerrainVertex(
				...
				(float)z,
				j % _uvScale * uCoordIncrementSize,//纹理坐标的计算
				i % _uvScale * vCoordIncrementSize);
...
bool Terrain::lightTerrain(D3DXVECTOR3* directionToLight)
{
	...
	float hScale = _numVertsPerCol / textureDesc.Height;
	float wScale = _numVertsPerRow / textureDesc.Width;
	DWORD* imageData = (DWORD*)lockedRect.pBits;
	for(int i = 0; i < textureDesc.Height-1; i++)
	{
		for(int j = 0; j < textureDesc.Width-1; j++)
		{
			...
			c *= computeShade(i*hScale, j*wScale, directionToLight);
...
float Terrain::computeShade(int cellRow, int cellCol, D3DXVECTOR3* directionToLight)
{
	...
	float c;
	float height = (heightA + heightB + heightC) / 3.0f;
	c = height / (_highest - _lowest) + 0.5;
...
void Terrain::GetPressPointInTerrain( D3DXVECTOR3 & pressp,const D3DXVECTOR3 &Eyeat,const D3DXVECTOR3 &pressonworld)
{
	float fTempH = getHeight(pressp.x,pressp.z);

	D3DXPLANE plane;

	D3DXVECTOR3 v1(1.0f,fTempH,1.0f);
	D3DXVECTOR3 v2(-1.0f,fTempH,1.0f);
	D3DXVECTOR3 v3(-1.0f,fTempH,-1.0f);
...
IDirect3DVertexBuffer9* Terrain::getVertexBuffer()
{
	return _vb;
}
IDirect3DIndexBuffer9* Terrain::getIndexBuffer()
{
	return _ib;
}