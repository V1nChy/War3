...
public:
	...
	virtual void resetParticle(Attribute* attribute) = 0;//重置一个粒子的属性，该函数设计为纯虚函数，其实现依赖于子类
	//添加一个粒子到系统中。
	virtual void addParticle();

	virtual void update(float timeDelta) = 0;//更新系统中的所有粒子，其实现依赖于具体子类

	virtual void preRender();//设置初始渲染状态，它必须在渲染之前被调用
	virtual void render();//显示系统中的所有粒子
	virtual void postRender();//恢复某些渲染状态
...
protected:
	virtual void removeDeadParticles();//移除死亡的粒子，子类可以修改（有些需要再利用）
...
{
public:
	Firework(D3DXVECTOR3* origin, int numParticles);//origin表示爆炸点
	void resetParticle(Attribute* attribute);
	void update(float timeDelta);
	void preRender();
	void postRender();
