...
CSkinMesh::CSkinMesh(IDirect3DDevice9* device)
{
	m_is_play_anim	  = true;
	m_device		  = NULL;
	...
	m_device = device;
	m_alloc_hierarchy = new cAllocateHierarchy(m_device);
...
HRESULT CSkinMesh::create(const char* filename)
{
...
VOID CSkinMesh::SetAddHeight(float fheigh)
{
	m_fHeight += fheigh;
}
...
ID3DXMesh* CSkinMesh::GetOrgMesh()
{
	return m_alloc_hierarchy->pOrgMesh;
}