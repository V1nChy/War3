...
HRESULT cAllocateHierarchy::GenerateSkinnedMesh(D3DXMESHCONTAINER_DERIVED* mesh_container)
{
	...
	pOrgMesh = mesh_container->pOrgMesh;
	return hr;
}
                                                ...
                                                LPD3DXMESHCONTAINER* ret_mesh_container) 
{
	*ret_mesh_container = NULL;
	...
	{
		hr = mesh_ptr->CloneMeshFVF(mesh_ptr->GetOptions(), fvf | D3DFVF_NORMAL, m_pDevice, &new_mesh_container->MeshData.pMesh);
	...
	if(num_materials > 0)
	{
		...
		for(UINT i = 0; i < num_materials; i++)
		{
			if(xmaterials_ptr[i].pTextureFilename != NULL)
			{
				...
				if(D3D_OK != D3DXCreateTextureFromFile(m_pDevice, strTexturePath, &new_mesh_container->ppTextures[i]) )
